...
, action_(action)
, target_(new GetSceneObjectCommand(scene_graph))
, gamepad_(gamepad)
{
}
...
void WidgetEventCommand::execute() {
   ...
   case WidgetOp::MouseMove:
      ...
      if (this->target_->get_widgets().size() > 0) {
         w = this->target_->get_widgets()[0];
      }
      ...
      if (this->target_->get_widgets().size() > 0) {
         w_prev = this->target_->get_widgets()[0];
      }
   ...
   case WidgetOp::MouseClick:
      ...
      if (this->target_->get_widgets().size() > 0) {
         this->target_->get_widgets()[0]->on_click();
      }
   break;
   case WidgetOp::MouseRelease:
      ...
      if (this->target_->get_widgets().size() > 0) {
         this->target_->get_widgets()[0]->on_release();
      }
   break;
   }
}
