...
void IsoMapBuilder::build_tile(Entity* tile) {
   if (!this->map_) {
      this->build_map();
   }

   // TODO: redo this
   //tile = TileFactory::inst()->create_tile(this->texture_manager_.get_texture(texture));
...
void IsoMapBuilder::build_grid(Grid* grid) {
   if (!this->map_) {
      this->build_map();
   }
